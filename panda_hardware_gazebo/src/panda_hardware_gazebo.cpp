...
#include <panda_hardware_gazebo/panda_hardware_gazebo.h>

namespace panda_hardware_gazebo
{
  using namespace hardware_interface;

  PandaHardwareGazebo::PandaHardwareGazebo()
    : gazebo_ros_control::RobotHWSim()
    , ns_("panda_hardware_gazebo/")
    , log_cnt_(0)
  {}


  bool PandaHardwareGazebo::initSim(const std::string& robot_namespace,
      ros::NodeHandle nh,
  ...
  void PandaHardwareGazebo::readSim(ros::Time time, ros::Duration period)
  {
    // 获取关节状态
  ...
  void PandaHardwareGazebo::writeSim(ros::Time time, ros::Duration period)
  {
    // 改变关节状态
    ...
    for(int i = 0; i <  sim_joints_.size(); i++)
    {
      std::string gazebo_jnt_name;
      gazebo_jnt_name = sim_joints_[i]->GetName();
      // ROS_INFO("gazebo_jnt_name:%s", gazebo_jnt_name.c_str());
      // wheels
      if(gazebo_jnt_name == virtual_rear_wheel_jnt_names_[INDEX_RIGHT])
      {
        ...
        //}
        // ROS_INFO("set joint %s to %f", gazebo_jnt_name.c_str(), rear_wheel_jnt_vel_cmd_);
      ...
      //   // ROS_INFO("set joint %s to %f", gazebo_jnt_name.c_str(), front_steer_jnt_pos_cmd_);
      ...
      else if(gazebo_jnt_name == virtual_front_steer_jnt_names_[INDEX_RIGHT])
      {
        ...
        sim_joints_[i]->SetPosition(0, front_steer_jnt_pos_cmd_, true);
#else
        sim_joints_[i]->SetPosition(0, front_steer_jnt_pos_cmd_);
#endif
      }
//       else if(gazebo_jnt_name == virtual_front_steer_jnt_names_[INDEX_LEFT])
//       {
...
//       }
  ...
  void PandaHardwareGazebo::CleanUp()
  {
    // wheel
    //-- wheel joint names
  ...
  void PandaHardwareGazebo::GetJointNames(ros::NodeHandle &_nh)
  {
    this->GetWheelJointNames(_nh);
    this->GetSteerJointNames(_nh);
  }

  void PandaHardwareGazebo::GetWheelJointNames(ros::NodeHandle &_nh)
  {
  ...
  void PandaHardwareGazebo::GetSteerJointNames(ros::NodeHandle &_nh)
  {
  ...
  void PandaHardwareGazebo::RegisterHardwareInterfaces()
  {
  ...
  void PandaHardwareGazebo::RegisterInterfaceHandles(
  ...
  void PandaHardwareGazebo::RegisterWheelInterface()
  {
    // actual wheel joints
  ...
  void PandaHardwareGazebo::RegisterSteerInterface()
  {
    // actual steer joints
    this->RegisterInterfaceHandles(
  ...
  void PandaHardwareGazebo::RegisterJointStateInterfaceHandle(
      hardware_interface::JointStateInterface& _jnt_state_interface,
      const std::string _jnt_name, double& _jnt_pos, double& _jnt_vel, double& _jnt_eff)
  {
  ...
  void PandaHardwareGazebo::RegisterCommandJointInterfaceHandle(
      hardware_interface::JointStateInterface& _jnt_state_interface,
      hardware_interface::JointCommandInterface& _jnt_cmd_interface,
  ...
  double PandaHardwareGazebo::ComputeEffCommandFromVelError(
  ...
  void PandaHardwareGazebo::GetCurrentState(std::vector<double>& _jnt_pos, std::vector<double>& _jnt_vel, std::vector<double>& _jnt_eff,
...
PLUGINLIB_EXPORT_CLASS(panda_hardware_gazebo::PandaHardwareGazebo, gazebo_ros_control::RobotHWSim)
